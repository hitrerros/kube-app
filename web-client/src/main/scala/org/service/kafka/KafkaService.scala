...
object KafkaService {
  ...
  private def processRecord[F[_]: Async](record: ConsumerRecord[String, CustomRecord])(logger: Logger[F]): F[Unit] =
    logger.info(s"=> App received record: ${record.value}")
  ...
  private def make[F[_]: Async](logger: Logger[F]): KafkaService[F] =
    new KafkaService[F] {
      override def receive: F[Unit] =
        KafkaConsumer
          ...
          .flatMap(_.stream)
          .evalMap { committable =>
            processRecord[F](committable.record)(logger).as(committable.offset)
          }
  ...
  def provider[F[_] : Async]: Resource[F, KafkaService[F]] =
    for {
      ...
      service = make[F](logger)
      _ <- service.receive.background // automatically start consuming
    } yield service
}